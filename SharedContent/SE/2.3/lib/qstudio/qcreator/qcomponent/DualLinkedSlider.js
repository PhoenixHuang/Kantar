/**
 * DualLinkedSlider Javascript File
 * Version : 1.3.0
 * Date : 2014-09-26
 *
 * DualLinkedSlider Component.
 * BaseClassType : Single
 * Requires rowArray dataset.
 * Confluence Link : https://docs.gmi-mr.com/confluence/display/midev/DualLinkedSlider+Component+Documentation
 *
 * Release 1.3.0
 ...
 *   - getRowSldrParams: Returns an object listing all the row slider level parameters.
 ...
 * - Added a new string parameter, compDefaultColors, which you can use to set component default colors
 * - Added a new string parameter, rowSldrBorderStyle, which you can use to set the slider widget border style
 * - Added a new numeric parameter, rowSldrBorderWidth, which you can use to set the slider widget border width
 * - Added a new color parameter, rowSldrBorderColor, which you can use to set the slider widget border color
 * - Added a new boolean parameter, rowSldrShowLabelBckgrnd, which you can use to toggle the display of the slider widget label background color
 * - Added a new color parameter, rowSldrLabelBckgrndColor, which you can use to set the slider widget label background color
 * - Added a new boolean parameter, rowOtherShowLabel, which when set true will display row input widget labels
 * - Removed numeric parameter, rowSldrLabelWidth, since the label width of slider widgets is now automatically set
 * - Removed numeric parameter, rowKantBtnLabelWidth. This value is now inherited from rowBtnWidth
 *
 */

var DualLinkedSlider = (function () {

    function DualLinkedSlider() {
        this.qStudioVar = {
    ...
    DualLinkedSlider.prototype = {
        ...
        description: function() {
            return 'DualLinkedSlider component description...';
        },
        ...
        initParams: function() {
            return {
                ...
                rowSldrHndleInitLoc: { name:"Slider Handle: Start Loc", value:50, description:"Slider handle starting position. Number is percent based so 50 would represent the halfway point of the slider track w/ 0 and 100 being the endpoints. ", type:"number", min:0, max:100, wgtref: 'rowsldr', wgtname:"handle_start_loc", order:93 },
            ...
            if (wgtName) {
                ...
                switch (value) {
                    ...
                    case 'rowsldr' :
                        ...
                        if (configObj.handle_start_loc < 0) { configObj.handle_start_loc = 0; }
                        if (configObj.handle_start_loc > 100) { configObj.handle_start_loc = 100; }
                        break;
                    default :
                        break;
                }
        ...
        __initGoogleChart: function() {
            ...
            this.googleUtility = {
                init: function() {
                    ...
                    for (i = 0; i < rlen; i += 1) {
                        if (respRef.rowRefArry[i]) {
                            var row = respRef.rowRefArry[i].row;
                            row.enabled(true);
                            if (row.type() === "slider") {
                                respRef.chartConfigObj.colors.push(rowArray[i].var2);
                                respRef.chartDataArry.push([rowArray[row.rowIndex()].label, respRef.rowRefArry[i].handle_start_loc]);
        ...
        create: function(parent, surveyContain) {
            ...
            var that = this,
                ...
                eventDown = (!isMSTouch) ?
                    "mousedown.duallinkedslider touchstart.duallinkedslider" :
                    ((window.PointerEvent) ? "pointerdown.duallinkedslider" : "MSPointerDown.duallinkedslider"),
                eventUp = (!isMSTouch) ?
                    "mouseup.duallinkedslider touchend.duallinkedslider" :
                    ((window.PointerEvent) ? "pointerup.duallinkedslider" : "MSPointerUp.duallinkedslider"),
                eventMove = (!isMSTouch) ?
                    "touchmove.duallinkedslider" :
                    ((window.PointerEvent) ? "pointermove.duallinkedslider" : "MSPointerMove.duallinkedslider"),
                eventCancel = (!isMSTouch) ?
                    "touchcancel.duallinkedslider" :
                    ((window.PointerEvent) ? "pointercancel.duallinkedslider" : "MSPointerCancel.duallinkedslider"),
                txtEvent = ("oninput" in compContain) ? "input.duallinkedslider" : "keyup.duallinkedslider",
                touchCoordArray = [],
                i = 0, rlen = rowArray.length,
                btnSliceIndex = -1;

            // To fix IE9 bug where input event does not fire on backspace/del
            if (QUtility.ieVersion() <= 9) { txtEvent = "keyup.duallinkedslider"; }
            ...
            if (btnSliceIndex !== -1) {
                ...
                if (!(this.qStudioVar.sliderArray.length > 1)) {
                    throw new Error("ERROR: Component requires 2 sliders.");
                }
            } else {
            ...
            compContain.id = "DualLinkedSliderComponent";
            compContain.dir = (!this.qStudioVar.isCompRTL) ? 'ltr' :'rtl';
            compContain.className = "qduallinkedslider_component";
            ...
            $(compContain).on(eventDown, '.qwidget_button', function(event) {
                ...
                var rowEle = event.currentTarget,
                    row = (respRef.rowRefArry[rowEle.getAttribute('rowIndex')]) ?
                        respRef.rowRefArry[rowEle.getAttribute('rowIndex')].row : null;

                if (row === null || (row && !row.enabled()) || that.qStudioVar.isUpdating) { return; }
        ...
        update: function(init) {
            ...
            var that = this,
                ...
                i = 0,
                slen = 2,   // restrict number of sliders to 2
                ...
                rowSldrMaxVal = params.rowSldrMaxVal.value;
            ...
            for (i = 0; i < slen; i += 1) {
                ...
                wgtNameObj.handle_start_loc = (i === 0) ?
                    wgtNameObj.handle_start_loc : (rowSldrMaxVal - wgtNameObj.handle_start_loc);
                respRef.sldrTotal += wgtNameObj.handle_start_loc;
                ...
                if (init) {
                    ...
                    respRef.rowRefArry.push({
                        row : rowWgt,
                        rangeValue : wgtNameObj.handle_start_loc,
                        handle_start_loc : wgtNameObj.handle_start_loc
                    });

                    // since the sliders are answered beforehand, push them into sldrRespArry
                    respRef.sldrRespArry.push(rowWgt);

                    rowWgt.isAnswered(true, wgtNameObj.width*(wgtNameObj.handle_start_loc/100), false);

                    // Add callback functions to slider
                    (function(sldr) {
                        sldr.slideMove(function() {
                            var sldrParams = sldr._params,
                                sldrSize = (sldrParams.direction.toLowerCase() === "horizontal") ?
                                    sldrParams.width : sldrParams.height,
                                sldrRangeValue = sldr._rangeValue(),
                                otherSldr = (sldrParams.rowIndex === 0) ? respRef.rowRefArry[1].row : respRef.rowRefArry[0].row,
                                otherSldrParams = otherSldr._params,
                                otherSldrRangeValue = otherSldr._rangeValue(),
                                otherSldrLoc = (sldrParams.direction.toLowerCase() === "horizontal" && !that.qStudioVar.isCompRTL) ?
                                    (sldrSize * ((rowSldrMaxVal - sldrRangeValue) / 100)) : sldrSize - (sldrSize * ((rowSldrMaxVal - sldrRangeValue) / 100));

                            // record slider range values
                            respRef.rowRefArry[sldrParams.rowIndex].rangeValue = sldrRangeValue;
                            respRef.rowRefArry[otherSldrParams.rowIndex].rangeValue = otherSldrRangeValue;

                            // move other slider @ the same time
                            otherSldr.isAnswered(true, otherSldrLoc, false);
                        });
                        ...
                        sldr.slideEnd(function() {
                            ...
                            // update sldrTotal
                            respRef.sldrTotal = sldrRangeValue + otherSldrRangeValue;

                            // call manageSlide on both sliders
                            that.manageSlide(sldr);
                            that.manageSlide(otherSldr);

                            // update google charts
                            if (chartType !== 'none') {
                                that.googleUtility.update(sldrParams.rowIndex, sldrRangeValue);
                                that.googleUtility.update(otherSldrParams.rowIndex, otherSldrRangeValue);
                            }
                        });
                    }(rowWgt));
                } else {
            ...
            for (i = 0; i < blen; i += 1) {
                ...
                if (init) {
                    ...
                    // Store reference of Row Widget for use w/ controller
                    respRef.rowRefArry[rowIndex] = { row: rowWgt };
                } else {
        ...
        manageSlide: function(slider) {
            ...
            // Answer slider
            respRef.chartDataArry[1] = [params.chartRemainText.value, 0];
            this.sendResponse(rowIndex, rangeValue);
            this.__setIsCompAnswered(respRef.sldrTotal === params.rowSldrMaxVal.value);
