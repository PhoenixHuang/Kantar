...
QVerticalLayout.prototype.update = function() {
    ...
    var that = this,
        ...
        isTouchMove = false,
        eventStr = (!isMSTouch) ?
            ("click.qverticallayout touchstart.qverticallayout touchend.qverticallayout touchmove.qverticallayout"):
...
QHorizontalLayout.prototype.update = function() {
    ...
    var that = this,
        ...
        isTouchMove = false,
        eventStr = (!isMSTouch) ?
            ("click.qhorizontallayout touchstart.qhorizontallayout touchend.qhorizontallayout touchmove.qhorizontallayout"):
...
QStudioAssetFactory.prototype.assetUpdate = {
    ...
    baseBckgrnd : function(bckgrnd, configObj) {
        ...
        bckgrnd.className = "qwidget_background";
        bckgrnd.style.position = "absolute";
        bckgrnd.style.display = "inline-block";
        bckgrnd.style.filter = "inherit";
    ...
    image : function(imageContain, configObj) {
        ...
        params.image = (QUtility.isString(configObj.image)) ? jQuery.trim(configObj.image) : "";
        params.width = (QUtility.isNumber(configObj.width) && configObj.width >= 1) ? configObj.width : 100;
        params.height = (QUtility.isNumber(configObj.height) && configObj.height >= 1) ? configObj.height : 100;
    ...
    label : function(labelContain, configObj) {
        ...
        labelContain.dir = (!params.isRTL) ? "LTR" : "RTL";
    ...
    textarea : function(textarea, configObj) {
        ...
        var evtCharCount = "keyup.count.widget blur.count.widget",
            params = {};
        ...
        if (params.max_char > 0) {
            if ("maxLength" in textarea) {
                ...
                var max_char = params.max_char;
                this.addEvent(textarea, evtCharCount , function(event) {
            ...
            ("maxLength" in textarea) ?
                ...
                this.removeEvent(textarea, evtCharCount);
        }
...
QStudioBtnAbstract.prototype.toggleMouseDown = function(value) {
    ...
    if (this._params.show_bckgrnd) {
        ...
        if (!this._params.show_bckgrnd_import) {
            ...
            cacheNodes.background.style.backgroundImage = (QUtility.ieVersion() < 9) ?
                "url(" + "" + ") " + "url(" + bckgrnd_import + ")" : "url(" + bckgrnd_import + ")";
            cacheNodes.background.style.filter = (value) ?
...
QStudioInputBtnAbstract.prototype.initEvents = function() {
    ...
    // Add TextArea events
    this.addEvent(textarea, this._events.click, function(event) {
        event.stopPropagation();
        if (event.type !== "touchmove") {
            if (event.type !== 'touchstart' && event.type !== "pointerdown" && event.type !== "MSPointerDown") {
...
function QStudioSliderAbstract() {
    this._events = {
        click : (!QUtility.isMSTouch()) ?
            ("click.widget touchstart.widget touchend.widget touchmove.widget"):
            ((!QUtility.isTouchDevice()) ? "click.widget" : ((window.PointerEvent) ? "pointerdown.widget pointerup.widget" : "MSPointerDown.widget MSPointerUp.widget")),
        ...
        if (event.type !== "touchmove") {
            if (event.type !== 'touchstart' && event.type !== "pointerdown" && event.type !== "MSPointerDown") {
                ...
                if (!isTouchMove) {
                    ...
                    if (event.type === "click") { that._cache._downEvent = event; }
                    that.slideInit(that._cache._downEvent, true);    // pass true to indicate method is being called from a track click
                    that.slideMove(that._cache._downEvent);
                    that.slideEnd(that._cache._downEvent);
                    delete that._cache._downEvent;
                }
            } else {
                that._cache._downEvent = event;
                isTouchMove = false;
            }
        } else {
            isTouchMove = true;
...
QStudioSliderAbstract.prototype.slideInit = function(event, fromTrack) {
    ...
    fromTrack = (typeof fromTrack === "boolean" && fromTrack) ? true : false;
...
QFlowBtn.prototype.update = function() {
    ...
    if (updateLabel) {
        ...
        if (labContainHeight > params.height) {
            ...
            cacheNodes.labelContain.style.height = labContainHeight + "px";
            cacheNodes.label.style.height = (labContainHeight - 0) + "px";
            cacheNodes.label.style.overflow = "hidden";
        }
...
QBaseInputBtn.prototype.update = function() {
    ...
    (!params.allow_click) ?
        this.addEvent(widget, this._events.click, function(event) { event.stopPropagation(); }):
        this.removeEvent(widget, this._events.click);
};
...
var qLightBoxSingleton = (function () {
    ...
    function QLightBox(parent, configObj) {
        ...
        this._events = {
            ...
            click : (!QUtility.isMSTouch()) ?
                ("click.lightbox mousedown.lightbox mouseup.lightbox touchstart.lightbox touchend.lightbox touchmove.lightbox"):
        ...
        $(window).on(this._events.resize, function(event) {
            if (lightBox.style.display === 'none') { return; }
            if (that._params.gallery_autosize) { that.resize(); }
            that.center();
        });
    }
    ...
    QLightBox.prototype.addWgt = function(wgt, configObj) {
        ...
        if (jQuery.inArray(wgt, wgtArray) === -1) {
            ...
            if (params.action_type !== "mouseover") {
                ...
                $(zoomBtn).css( {
                    ...
                    'margin-top' : params.zoom_top + "px",
                    'margin-left' : params.zoom_left + "px",
                ...
                // click event
                $(zoomBtn).on(this._events.click, function(event) {
                    event.stopPropagation();
                    if (event.type !== "touchmove") {
                        if (event.type !== "mousedown" && event.type !== "mouseup" && event.type !== 'touchstart' && event.type !== "pointerdown" && event.type !== "MSPointerDown") {
                            if (event.type !== "click") { event.preventDefault(); }
                            if ((!wgt.enabled() && !wgt.cache()._enableExtEvt) || wgt.isDrag()) { return; }
                            if (!isTouchMove) { that.show(wgt); }
                        } else {
                            isTouchMove = false;
                        }
                    } else {
    ...
    QLightBox.prototype.removeWgt = function(wgt) {
        ...
        if (spliceIndex !== -1) {
            ...
            if (zoomBtn) {
                $(zoomBtn).off(this._events.click);
    ...
    QLightBox.prototype.show = function(wgt) {
        ...
        var images = cacheNodes.gallery.getElementsByTagName('img');
        for (var i = 0, image; image = images[i]; i++) {
            cacheNodes.gallery.removeChild(image);
        }

        // prevent pinching on touch devices
        if (QUtility.isTouchDevice()) {
            if (this._cache.metaTag === undefined) {
                this._cache.metaTag = document.createElement('meta');
                this._cache.metaTag.id = "lightbox_meta_scale";
                this._cache.metaTag.name = "viewport";
                document.getElementsByTagName('head')[0].appendChild(this._cache.metaTag);
            }
        ...
        // image css style
        img.style.cssText = "filter: inherit; width: auto; height: auto; max-width: 100%; max-height: 100%;";
        img.style.padding = params.gallery_padding + "px";
        img.style.backgroundColor = "#F5F5F5";
        img.style.border = params.gallery_border_width + "px " + params.gallery_border_style + " #" + params.gallery_border_color;
        ...
        // append new image
        (cacheNodes.gallery.firstChild) ?
            cacheNodes.gallery.insertBefore(img, cacheNodes.gallery.firstChild):
            cacheNodes.gallery.appendChild(img);

        // position gallery image and fade in
        lightBox.style.display = "block";
        $(cacheNodes.gallery).css({ "opacity": 0 });
        this.resize();
        this.center();
        $(cacheNodes.gallery).stop();
        $(cacheNodes.gallery).animate({ "opacity": 1 }, 300);
    };
    ...
    QLightBox.prototype.resize = function() {
        ...
        var wind = window,
            doc = document,
            cacheNodes = this._cache.nodes,
            windowWidth = Math.min(((wind.innerWidth !== undefined) ? wind.innerWidth : $(wind).width()), $(wind).width()),
            windowHeight = Math.min(((wind.innerHeight !== undefined) ? wind.innerHeight : $(wind).height()), $(wind).height());
        ...
        cacheNodes.gallery.style.width = ((this._params.gallery_autosize) ? windowWidth : this._params.gallery_max_width) + "px";
        cacheNodes.gallery.style.height = ((this._params.gallery_autosize) ? windowHeight : this._params.gallery_max_height) + "px";
    };
    QLightBox.prototype.center = function() {
        if (!this._widget || this._widget.style.display === 'none') { return false; }
        var wind = window,
            cacheNodes = this._cache.nodes,
            imageOuterWidth = $(cacheNodes.gallery.firstChild).outerWidth(),
            imageOuterHeight = $(cacheNodes.gallery.firstChild).outerHeight(),
            x = $(wind).scrollLeft() + ($(wind).width() / 2) - (imageOuterWidth / 2),
            y = $(wind).scrollTop() + ($(wind).height() / 2) - (imageOuterHeight / 2);
        ...
        cacheNodes.gallery.style.left = x + 'px';
        cacheNodes.gallery.style.top = y + "px";

        // position close button
        cacheNodes.closeBtn.style.left = imageOuterWidth + "px";
    };
    ...
    return {
        ...
        destroy: function() {
            ...
            $(instance._widget).off(instance._events.click);
