 ...
 * - Added new option, 'Multiple Choice' to string parameter, compQuestionType.
 * - Added a new string parameter, rowBtnDefaultType, which you can use to set a default button widget type. Options include 'base', 'text' or 'flow'
 * - Removed boolean parameter, rowBtnShowValueLabel. The string parameter, rowBtnValLabDispType, currently has option a 'none' which does the same thing.
 * - You can now include a RadioCheck button widget, which will act like a 'Don't Know' option, to accompany the column slider.
 ...
 * - When a row object is placed on the slider scale you will no longer be able to drag past the endpoints.
 *
 */
    ...
    DragnFlag.prototype = {
        ...
        initParams: function() {
            return {
                ...
                colRadChckShowImpRadio: { name:"Column Don't Know: Show Import Bckgrnd", value:false, description:"If set true, buttons which behave like Radio buttons will display an imported background image.", type:"bool", wgtref: 'dkbtn', wgtname:"radchkbtn_rad_show_import", order:49 },
        ...
        create: function(parent, surveyContain) {
                ...
                if (rowWgt.enabled()) {
                    ...
                    $(doc).on(moveEvent, function(event) {
                        ...
                        if (ex >= respRef.eventCache.trackBounds.left && ex <= respRef.eventCache.trackBounds.right && ey >= respRef.eventCache.trackBounds.top && ey <= respRef.eventCache.trackBounds.bottom) {
                            ...
                            /*rowWgt.widget().style.top =
                                ((respRef.eventCache.trackTopOffset - respRef.eventCache.rowWrapTopOffset) -
                                    respRef.eventCache.rowHeight - rowWgt.flagStickHeight() +
                                    (respRef.eventCache.sliderParams.height*.5 - respRef.eventCache.sliderParams.track_border_width)) + "px";*/
                        }
            ...
            if (respRef.dkWgt) {
                ...
                var eventStr = (!isMSTouch) ?
                        ("click.dragnflag touchstart.dragnflag touchend.dragnflag touchmove.dragnflag"):
                        ((!isTouchDevice) ? "click.dragnflag" : ((window.PointerEvent) ? "pointerdown.dragnflag pointerup.dragnflag" : "MSPointerDown.dragnflag MSPointerUp.dragnflag")),
                    isTouchMove = false;
