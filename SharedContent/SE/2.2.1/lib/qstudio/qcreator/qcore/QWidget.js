...
function QStudioInputBtnAbstract() {
    this._events = {
        ...
        click : "click.widget mousedown.widget touchstart.widget"
    };
}
...
function QStudioSliderAbstract() {
    this._events = {
        click : "click.widget",
        down : "mousedown.widget touchstart.widget",
        move : "mousemove.widget touchmove.widget",
        up : "mouseup.widget touchend.widget"
    };
}
...
QStudioSliderAbstract.prototype.initEvents = function() {
    ...
    this.addEvent(cacheNodes.handleContain, this._events.down, function(event) {
        ...
        if (!that._cache._enableBool || that.isDrag()) { return; }
...
QStudioSliderAbstract.prototype.slideInit = function(event, fromTrack) {
    ...
    event = (event.type.indexOf("touch") !== -1 && event.originalEvent.targetTouches) ?
        event.originalEvent.targetTouches[0] : event;
...
function QBaseBtn(parent, configObj) {
    ...
    widget.style.cssText += ';'.concat("touch-action: none;");
...
function QBaseInputBtn(parent, configObj) {
    ...
    // add touch-action none to textarea
    textarea.style.cssText += ';'.concat("-ms-touch-action: none;");
    textarea.style.cssText += ';'.concat("touch-action: none;");
...
function QBaseSlider(parent, configObj) {
    ...
    track.style.cssText += ';'.concat("-ms-touch-action: none;");
    track.style.cssText += ';'.concat("touch-action: none;");
    ...
    QLightBox.prototype.addWgt = function(wgt, configObj) {
        ...
        if (jQuery.inArray(wgt, wgtArray) === -1) {
            ...
            if (params.action_type !== "mouseover") {
                ...
                // click event
                $(zoomBtn).on("click.lightbox mousedown.lightbox touchstart.lightbox", function(event) {
                    event.stopPropagation();
                    if (event.type === 'click') {
                        if ((!wgt.enabled() && !wgt.cache()._enableExtEvt) || wgt.isDrag()) {
                            return;
                        }
                        that.show(wgt);
                    }
