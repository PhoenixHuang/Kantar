...
var intouchClickSpot = function() {
    try {
        ...
        if (globalCheck) {
            ...
            if (typeof autoSubmitMilliSec != "undefined" && autoSubmitMilliSec != ""  && parseInt(minClicks) > 0) pageLayout.next.hide();
            if (typeof minClicks != "undefined" && minClicks != ""  && parseInt(minClicks) > 0) pageLayout.next.hide();
        }
		...
		window.addEventListener("orientationchange", function() {
		 ...
		 window.setTimeout(function() {
						...
						$(intouchClickSpotApp.appCanvas).unbind("click", intouchClickSpotApp.spotTheClick);
						 ...
						 that.preloaddata=$("#_Q0").val();	
						 intouchClickSpotApp.clearTheCanvas();
						 $("#_Q0").val(that.preloaddata);						 
        ...
        e.load(function() {
            if (useImageDimensions) {
				...
				that.width_ratio = oldWidth / this.width;
				that.height_ratio = oldHeight / this.height;
            ...
            $("#CSUndoBtn").css({
                ...
                color: buttonTextColor
						...
						that.xyz=$("#_Q0").val();
						  var e = "";
						 $.each(that.xyz.split(","), function( index, value ) {
							  var width = window.innerWidth || document.documentElement.clientWidth;
							var height = window.innerHeight || document.documentElement.clientHeight;
							  var split=value.split("~");
							  if(width> height)
								{
									e += split[0] + "~" + Math.round(split[1]/that.width_ratio) + "~" + Math.round(split[2]/that.height_ratio)  + "~" + split[3] + ","					
								}
								else
								{
									e += split[0] + "~" + Math.round(split[1]/that.width_ratio) + "~" + Math.round(split[2]/that.height_ratio)  + "~" + split[3] + ","
            ...
            if (globalCheck) {
                ...
                if(that.preloaddata=="No Data Captured~NaN~NaN~undefined")
				  that.preloaddata="";
				if (that.preloaddata != "" && that.preloaddata != "No Data Captured") {	
					intouchClickSpotApp.loadPreData(that.preloaddata)
                }
            }
        });
...
intouchClickSpot.prototype.canvasUndo = function() {
    try {
        ...
        intouchClickSpotApp.currClickCnt--;
        if (intouchClickSpotApp.currClickCnt < parseInt(minClicks) && parseInt(minClicks) > 0) pageLayout.next.hide();
        intouchClickSpotApp.clickSpotCanvasContext.clearRect(0, 0, intouchClickSpotApp.appCanvas.width, intouchClickSpotApp.appCanvas.height);
        intouchClickSpotApp.clickSpotCanvasContext.drawImage(intouchClickSpotApp.imageObj, 0, 0, clickSpotAppWidth, clickSpotAppHeight);
    ...
    try {
        ...
        if (intouchClickSpotApp.spotCollectionArr.length > 0) {
            for (var t = 0; t < intouchClickSpotApp.spotCollectionArr.length; t++) {
			    //console.log(Math.round(intouchClickSpotApp.spotCollectionArr[t].csXPos)+"-->"+Math.round(intouchClickSpotApp.spotCollectionArr[t].csYPos) )
			    //console.log(Math.round(intouchClickSpotApp.spotCollectionArr[t].csXPos*this.width_ratio)+"-->"+Math.round(intouchClickSpotApp.spotCollectionArr[t].csYPos*this.height_ratio) )
                e += intouchClickSpotApp.spotCollectionArr[t].spotNum + "~" + Math.round(intouchClickSpotApp.spotCollectionArr[t].csXPos*this.width_ratio) + "~" + Math.round(intouchClickSpotApp.spotCollectionArr[t].csYPos*this.height_ratio)  + "~" + intouchClickSpotApp.spotCollectionArr[t].elapsedTime + ","
            }
            e = e.substr(0, e.length - 1);
            $("#_Q0").val(e)
			console.log(e);
