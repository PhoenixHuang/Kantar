    ...
    LinkedSlider.prototype = {
        ...
        create: function(parent, surveyContain) {
            ...
            var that = this,
                ...
                eventDown = (!isMSTouch) ?
                    "mousedown.linkedslider touchstart.linkedslider" :
                    ((window.PointerEvent) ? "pointerdown.linkedslider" : "MSPointerDown.linkedslider"),
                eventUp = (!isMSTouch) ?
                    "mouseup.linkedslider touchend.linkedslider" :
                    ((window.PointerEvent) ? "pointerup.linkedslider" : "MSPointerUp.linkedslider"),
                eventMove = (!isMSTouch) ?
                    "touchmove.linkedslider" :
                    ((window.PointerEvent) ? "pointermove.linkedslider" : "MSPointerMove.linkedslider"),
                eventCancel = (!isMSTouch) ?
                    "touchcancel.linkedslider" :
                    ((window.PointerEvent) ? "pointercancel.linkedslider" : "MSPointerCancel.linkedslider"),
                txtEvent = ("oninput" in compContain) ? "input.linkedslider" : "keyup.linkedslider",
                touchCoordArray = [],
                i = 0, rlen = rowArray.length,
                btnSliceIndex = -1;

            // unescape all text w/ use for chart
            var textArea = doc.createElement('textarea');
            textArea.innerHTML = params.chartRemainText.value;
            params.chartRemainText.value = textArea.value;
            for (var k = 0; k < rlen; ++k) {
                textArea.innerHTML = rowArray[k].label;
                rowArray[k].label = textArea.value;
            }
            textArea = null;
