    ...
    DragnDrop.prototype = {
        ...
        initParams: function() {
            return {
                ...
                colDzoneListFontSize: { name:"Column Bucket: List Button Font Size", value:12, description:"Row list button label font size, in pixels.", type:"number", min:5, wgtref: 'coldzone', wgtname:"list_label_fontsize", order:21 },
        ...
        create: function(parent, surveyContain) {
            ...
            $(compContain).on(downEvent, '.qwidget_button', function(event) {
                ...
                $.event.trigger({
                    ...
                    eventType : "kantar_drag_init",
                    row : rowNode,
                    action: "dragging_init"
                });
                ...
                $(doc).on(upEvent, function(event) {
					...
					//Custom code added by Kantar 
					$.event.trigger({type:'SurveyEngineEvent', eventType: 'mb_dragged', message: 'Drag and Drop - Item Dragged Completed'});
                });
            });
                ...
                if (dkWgt.isAnswered()) {
                    for (var i = 0; i < rlen; i += 1) {
                        ...
                        if (rowWgt && rowWgt.colIndex() === 0) {
                            this.sendResponse(rowWgt, dkWgt, false);
                        }
                    }
                ...
                for (var i = 0; i < rlen; i += 1) {
                    ...
                    if (rowWgt) {
                        ...
                        if (rowWgt.colIndex() === 0) {
                            this.sendResponse(rowWgt, dkWgt, true);
                        }
                    }
                }
                ...
                if (jQuery.inArray(rowWrap, rowRefArry) === -1) {
                    if (bucketCapVal === -1 || (bucketCapVal === 1 || bucketQuery.length < bucketCapVal)) {
                        ...
                        if (rowBucket) {
                            ...
                            this.sendResponse(rowWgt, rowBucket, false);      // *SEND RESPONSE
                        }
                        ...
                        if (rowWgt.isRadio()) {
                            for (var i=0, drlen=bucketQuery.length; i<drlen; i+=1) {
                                ...
                                this.sendResponse(bucketRow, bucketWgt, false);    // *SEND RESPONSE
                            ...
                            if (rowRadioWgt && rowRadioWgt.isRadio()) {
                                ...
                                this.sendResponse(rowRadioWgt, bucketWgt, false);    // *SEND RESPONSE
                        ...
                        if (bucketCapVal === 1 && bucketQuery.length > 0) {
                            ...
                            this.sendResponse(respWgt, bucketWgt, false);   // *SEND RESPONSE
                        ...
                        this.sendResponse(rowWgt, bucketWgt, true);        // *SEND RESPONSE
        ...
        sendResponse: function(rowWgt, colWgt, ansVal) {
            var rowIndex = rowWgt.rowIndex(),
                columnIndex = colWgt.colIndex();
