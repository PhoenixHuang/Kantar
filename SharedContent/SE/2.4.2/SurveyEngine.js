...
var SurveyPage = function () {
	...
	this.toolVersion;
	/** @property {Number} orientation - orientation of the device*/
		...
		$.each(that.surveyData,function(index,json){
		   var thisPath;		   
			...
			if (customToolFlag==false)
			   thisPath =that.toolsPath+json.CustomProps.flaMetaType.toLowerCase()+"/" +json.CustomProps.flaMetaType.toLowerCase()+"_"+data[json.CustomProps.flaMetaType.toLowerCase()].version+ '.js';
			dependencies.push({
                'url':thisPath,
                'type':'script'
            });
...
SurveyPage.prototype.initOptions = function() {
	...
	this.initOrientation=("onorientationchange" in window)?(($(window).width()>$(window).height())?90:0):(($(window).width()>$(window).height())?90:0);
    switch(pageLayout.deviceType) {
        case "LARGETABLET":
        case "MEDIUMTABLET":
        case "SMALLTABLET":
        case "SMARTPHONETOUCH":
				if(this.initOrientation==0||this.initOrientation==180){
...
SESurveyTool.prototype = {
    ...
    init : function(questionContainers, json, globalOpts) {
		...
		this.orientation=("onorientationchange" in window)?(($(window).width()>$(window).height())?90:0):(($(window).width()>$(window).height())?90:0);
    ...
    create: function(){
		...
		dependencies.push({
		        'type':'script',
                'url':surveyPage.path + 'lib/qstudio/qcreator/qcore/1/QWidget.js'
    ...
    config: function() {
		...
		for (var i in that.customProps) {
			if (i.toLowerCase()=="opcodatasetpath"){				
				$('head').append("<script type='text/javascript' src='"+pageLayout.imageCache+this.customProps[i]+"'></script>");
				$.extend(this.options,opcoToolOptions());
			}
