...
dynamiclist.prototype.hideTheMainObjs = function ()
{
	try
	{
		...
		for(var eachQ=0;eachQ<numOfQuestionsArr.length;eachQ++)
		{
			...
			for(var eachQCat=0;eachQCat<theListOfQCategories.length;eachQCat++)
			{
				...
				radioBtnObjArr.push({RadioBtnObj:theListOfQCategories[eachQCat],RadioBtnLabel:eachRadioButtonLabel.toUpperCase(),RadioButtonID:numOfQuestionsArr[eachQ],RadioButtonName:eachCategoryName,RadioButtonOtherName:eachOtherCategoryName})
				//$(theListOfQCategories[eachQCat]).parent().hide();				
				if(otherTextStr=="")
				{
					if($(theListOfQCategories[eachQCat]).attr('checked') == 'checked')
					{					
						preSelectedArr.push({SelectedPreValue:eachRadioButtonLabel,SelectedObj:"#"+numOfQuestionsArr[eachQ]})
					}
				}				
			}
...
dynamiclist.prototype.createTheInputElements = function (refObj,qName,qIndex,defText)
{
	try
	{
...
dynamiclist.prototype.buildOptionsArr = function ()
{
	try
	{
...
dynamiclist.prototype.loadTheAutoCompCSS = function ()
{
	try
	{
