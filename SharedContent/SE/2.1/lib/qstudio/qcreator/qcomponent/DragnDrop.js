 ...
 *
 * Release 1.3.0
 * - Removed previous restrictions when using colContainOptHalign/colContainOptValign parameters. Parameters will now apply to both horizontal & vertical layout directions.
 * - Added new parameter rowContainOptHalign which horizontally aligns row options. Options include 'left', 'right', & 'center'.
 * - Added new parameter rowContainOptValign which vertically aligns row options. Options include 'top', 'bottom', & 'middle'.
 * - Fixed bug were text button widgets were being incorrectly sized during autosizing.
 * - Introduced rowColIndex property for setDimenResp method for 'Multi' question type. The rowColIndex property should be the row column index.
 *
 */
        ...
        create: function(parent, surveyContain) {
            ...
            $(compContain).on(downEvent, '.qwidget_button', function(event) {
                ...
                if (rowWgt.enabled()) {
                    ...
                    rowNode.dragY += ((scrollOffset >= 0) ? scrollOffset : 0);
                    ...
                    $(doc).on(moveEvent, function(event) {
                        ...
                        var scrollOffset = $(surveyContain).scrollTop(),
                            ex = (touchEvent.pageX - rowNode.dragX),
                            ey = (touchEvent.pageY - rowNode.dragY) + ((scrollOffset >= 0) ? scrollOffset : 0);
            ...
            if (jQuery.isArray(respArry)) {
                ...
                while(respArry.length !== 0)
                {
                    ...
                    if (rowIndex >= 0 && rowIndex < rowArray.length)
                    {
                        ...
                        if (isMulti && QUtility.isNumber(rowColIndex)) {
                            if (rowColIndex >= 0 && rowColIndex < respRef.colArry.length) {
                                row = respRef.rowArry[(rowIndex * colArryLen) + rowColIndex].row;
                            }
                        } else {
                            row = respRef.rowArry[rowIndex].row;
                        }

                        if (row && (colIndex >= 0 && colIndex < respRef.colArry.length))
                        {
