 ...
 */
/**
Fixed ios 10 touch event issues
**/
    ...
    RowPicker.prototype = {
        ...
        create: function(parent) {
            ...
            var that = this,
                ...
                eventDown = (!isMSTouch) ?
                    "mousedown.rowpicker touchstart.rowpicker" :
                    ((window.PointerEvent) ? "pointerdown.rowpicker" : "MSPointerDown.rowpicker"),
                eventUp = (!isMSTouch) ?
                    "mouseup.rowpicker touchend.rowpicker" :
                    ((window.PointerEvent) ? "pointerup.rowpicker" : "MSPointerUp.rowpicker"),
                eventMove = (!isMSTouch) ?
                    "touchmove.rowpicker" :
                    ((window.PointerEvent) ? "pointermove.rowpicker" : "MSPointerMove.rowpicker"),
                eventCancel = (!isMSTouch) ?
                    "touchcancel.rowpicker" :
                    ((window.PointerEvent) ? "pointercancel.rowpicker" : "MSPointerCancel.rowpicker"),
        ...
        manageClick: function(row) {
            ...
            if (capType !== "none" && this.qStudioVar.isDC && dcProxy) {
                var respLen = respRef.respArry.length,
                    capMet = (capType === "hard") ?
                        ((respLen > 0 && respLen === capVal) || (row.isRadio() && respLen > 0)):
                        ((respLen > 0 && respLen <= capVal) || (row.isRadio() && respLen > 0));

                if (respRef.capMet !== capMet) {
                    respRef.capMet = capMet;
                    dcProxy[(capMet) ? "showNextButton" : "hideNextButton"]();
                }
            }
        },
