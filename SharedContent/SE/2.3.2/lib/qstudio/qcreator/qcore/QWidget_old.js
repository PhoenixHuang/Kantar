...
QStudioInputBtnAbstract.prototype.initEvents = function() {
    ...
    // remove textarea events
    var cleanup = function() {
        $(textarea).off(that._events.cancel);
        $(textarea).off(that._events.up);
    ...
    this.addEvent(textarea, this._events.down, function(event) {
        event.stopPropagation();
        if (event.type === "mousedown" || event.type === "touchstart" || event.type === "pointerdown" || event.type === "MSPointerDown") {
            if (event.type !== "mousedown") {
                // touchcancel event
                $(textarea).on(that._events.cancel, function(event) {
                    event.stopPropagation();
                    cleanup();
                });
            ...
            // touchend event
            $(textarea).on(that._events.up, function(event) {
                event.stopPropagation();
...
var qLightBoxSingleton = (function () {
    ...
    function QLightBox(parent, configObj) {
        ...
        // helper function for lightBox event
        var cleanup = function() {
            // remove all possible events for zoomBtn
            $(lightBox).off(that._events.cancel);
            $(lightBox).off(that._events.up);
            $(doc).off(that._events.move);
            $(doc).off(that._events.up);
        };

        // add click/tap event
        $(lightBox).on(that._events.down, function(event) {
            event.stopPropagation();
            if (event.type === "mousedown" || event.type === "touchstart" || event.type === "pointerdown" || event.type === "MSPointerDown") {
                if (event.type !== "mousedown") {
                    // touchcancel event
                    $(lightBox).on(that._events.cancel, function(event) {
                        event.stopPropagation();
                        cleanup();
                    });
                ...
                // touchend event
                $(lightBox).on(that._events.up, function(event) {
                    event.stopPropagation();
                    if (event.type === "mouseup") {
                        cleanup();
