...
autocomplete.prototype.build = function() {
    ...
    $.each(this.inputs, function(i, e) {
        ...
        label = el.parent().find('label').clone();
        var rowDescription = $.trim(label.text().split("|")[0]);
        if(el.attr('isexclusive')=="false" && el.attr('otherid')=="" && !el.attr('otherinput')){
			that.values.push(rowDescription);
		}
        if (el.is(":checked") && el.attr('isexclusive')=="false" && el.attr('otherid')=="" && !el.attr('otherinput')) {
            active = "active";
...
autocomplete.prototype.render = function() {
	...
	$('[otherid]:not([otherid=""])').each(function(){
		$(this).parent().parent().clone().appendTo(that.component);
		$(this).parent().parent().remove();		
	})
	$('[isexclusive="true"]').each(function(){
		$(this).parent().parent().clone().appendTo(that.component);
		$(this).parent().parent().remove();		
	})
