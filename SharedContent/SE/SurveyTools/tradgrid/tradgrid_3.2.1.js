...
tradgrid.prototype.getDependencies = function() {
    if (this.getProperty("answertype") != null) {
		return [{
			"type": "script",
			"url": pageLayout.resolveFilePath(surveyPage.path + "lib/ILayouts/gm/" + surveyPage.toolVersion.tradgrid.idversion + "/Rules/tg_" + this.getProperty("answertype") + ".js")
		}];
	}else{
		return [];
	}
    ...
    if (this.getProperty("answertype") != null) {
        ...
        function loadrules() {
            ...
            $.each(eval("gm_" + that.getProperty("answertype").replace(/-/g, "_") + "(rs,cs)"), function(index, value, url) {
                if (value.c) {
                    if (typeof surveyPlatform == "undefined") { // This code belongs to Dimensions
                        that.url = pageLayout.resolveFilePath(surveyPage.path + "lib/ILayouts/gm/" + surveyPage.toolVersion.tradgrid.idversion + value.s);
                    } else if (surveyPlatform == "Nfield") { // This code belongs to Nfield
                        that.url = pageLayout.resolveFilePath(pageLayout.sharedContent + "SE/lib/ILayouts/gm/" + surveyPage.toolVersion.tradgrid.idversion + value.s);
                    }
		...
		loadrules();
        /*$.when(
            $.getScript(pageLayout.resolveFilePath(surveyPage.path + "lib/ILayouts/gm/" + surveyPage.toolVersion.tradgrid.idversion + "/Rules/gm_" + this.getProperty("answertype") + ".js"))
        ).then(loadrules);*/
    } else {
