...
rowpicker.prototype.build = function() {
    ...
    $.each(this.inputs, function(i, e) {
		...
		if (typeof surveyPlatform == "undefined") {
			if (that.getProperty("row$displaytitle") == false && !(el.attr('isexclusive') == "true" || el.attr('otherid') != '')){
				rowArray[i].title = "";
			}
		}else if (surveyPlatform == "Nfield" ) {
			if (that.getProperty("row$displaytitle") == false && !(isexclusive == true || (typeof label.text().split("|")[1] != "undefined" && label.text().split("|")[1].toLowerCase().trim().indexOf("other") > -1))){
				rowArray[i].title = "";
			}
		}		
