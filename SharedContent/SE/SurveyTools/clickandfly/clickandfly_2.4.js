...
clickandfly.prototype.build = function(){
		...
		//code to block clickandfly if the response ID's are not unique
		var totalRadioArr = [], uniqueRadioArr = [];
		$('.mrSingle, .mrMultiple').each(function () {
			var thisRadioValue = $(this).attr('value');
			totalRadioArr.push(thisRadioValue);
			if ($.inArray(thisRadioValue,uniqueRadioArr)===-1) {
				uniqueRadioArr.push(thisRadioValue);
			}
		});
		if (totalRadioArr.length !== uniqueRadioArr.length) {
		    var errorMessage;
		   errorMessage= this.customProps.hasOwnProperty("uniqueIdErrorMsg")?this.customProps.uniqueIdErrorMsg:"Click and Fly requires all response ID's to be unique.";
			//handleSurveyEngineError(new Error("Click and Fly requires all response ID's to be unique."));
			handleSurveyEngineError(new Error(errorMessage));
		}
        ...
        $.each(this.inputs, function (i, e) {
			...
			if(typeof surveyPlatform=="undefined"){
				var labelData = label.html();
				var descriptionData = label.text();
			}else if(surveyPlatform=="Nfield"){
				var labelData = label.html().split("|")[0]  // this gets the styles from the html
				var descriptionData = label.text().split("|")[0]  // just grab the text for tooltip purposes
			}
            // Create the row array
            rowArray.push(
                {	id: el.attr('value'),
                    label: labelData,  // this gets the styles from the html
                    description: descriptionData,  // just grab the text for tooltip purposes
...
clickandfly.prototype.toolOptions = function() {
        $.extend(this.options, eval("this.options."+clickandfly.prototype.type()));
