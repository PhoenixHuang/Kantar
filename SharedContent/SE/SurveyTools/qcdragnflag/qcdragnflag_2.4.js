...
qcdragnflag.prototype.getDependencies = function(){
    return [
        {'type':'script', 'url' : surveyPage.path + 'lib/qstudio/qcreator/qcomponent/DragnFlag/DragnFlag.js'}
...
qcdragnflag.prototype.setResponses = function (){
    ...
    $.each($(response.Value), function(i, e) {
		if(typeof surveyPlatform=="undefined"){      // This code belongs to Dimensions
			var rowid=e.split("_");
			that.inputs.filter('[rowid='+rowid[0]+'][colid='+response[e].columnValue+']').val($.makeArray(that.inputs[response[e].columnValue].value));
		}else if(surveyPlatform=="Nfield"){     // This code belongs to Nfield
			var colvalue=that.inputs[response[e].columnValue].value.split('-');
			var subqid=e.split('-');
			var rowid=subqid[1].split("_");
			that.inputs.filter('[rowid='+rowid[0]+'][colid='+response[e].columnValue+']').val($.makeArray(subqid[0]+'-'+colvalue[1]));
		}
    });
}
