...
dynamicgrid.prototype.build = function() {
    ...
    $.each(this.columnheaders, function(i, e) {
		...
		if (typeof surveyPlatform == "undefined") {
			if (that.getProperty("column$displaytitle") == false && !(isexclusive|| $(inputs[i]).attr('otherid') != '')){
				colArray[i].title="";
			}
		}else if (surveyPlatform == "Nfield") {
			if (that.getProperty("column$displaytitle") == false && !(isexclusive == true || (typeof label.text().split("|")[1] != "undefined" && label.text().split("|")[1].toLowerCase().trim().indexOf("other") > -1))){
				colArray[i].title="";
			}
		}
...
dynamicgrid.prototype.toolOptions = function() {
    ...
    return {
        ...
        column: {
            ...
            inputonly: true,
			sldrsnap:true,
            extrasmall: {sldrendoffset: 2, displayimg: true,center:true,sldrendimgheight:35,sldrdisplayimg:true},
            small: {sldrendoffset: 2, displayimg: true,center:true,sldrendimgheight:35,sldrdisplayimg:true},
            medium: {sldrendoffset: 2, displayimg: true,center:true,sldrdisplayimg:true},
            large: {sldrendoffset: 2, displayimg: true,center:true,sldrdisplayimg:true},
            extralarge: {sldrendoffset: 2, displayimg: true,center:true,sldrdisplayimg:true}
            //pricolor:"#9FCC3B",
            //pridarkcolor:"#9FCC3B"
        },
        rowGrid: {
