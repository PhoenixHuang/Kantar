...
dynamicgrid.prototype.build = function() {
    ...
    this.toolParams = {
        ...
        next: (that.subquestions.length==1)?$('.tempNext'):document.getElementById("dgNext"),
        back: (that.subquestions.length==1)?$('.tempPrev'):document.getElementById("dgPrev")
		   ...
		   $.each(eval("sm_"+that.getProperty("answertype").replace(/-/g , "_")+"(rs,cs)"), function( index, value, url) {
			  if (value.c){
			   if (typeof surveyPlatform == "undefined") { // This code belongs to Dimensions
					...
					that.url=pageLayout.resolveFilePath(pageLayout.scriptContent+"SE/lib/ILayouts/dg/"+surveyPage.toolVersion.dynamicgrid.idversion+value.s);
				}
...
dynamicgrid.prototype.navNextButton = function() {
    ...
    if (this.isstudio) {
        return "<a id='dgNext' class='hoverable theme-standard-font-color3' style='display: block;'><img src='" + pageLayout.themePath + "images/Next.png' height='56'></a>";
    } else {
...
dynamicgrid.prototype.navPrevButton = function() {
    ...
    if (this.isstudio) {
        return '<a id="dgPrev" class="theme-standard-font-color1 hoverable" style="display: block;"><img src="' + pageLayout.themePath + 'images/Prev.png" height="56"></a>';
    } else {
