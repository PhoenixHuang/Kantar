...
mediaupload.prototype.render = function() {
    ...
    $('#' + fileId).change({
        ...
        if (selectedfile != undefined) {
            ...
            if (allowMedia == true) {
                if (videoType != null && fileSize > 60) {
                    pageLayout.hideLoader();
                    componentContainer.find('.mrErrorText').remove();
                    componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\">" + videofilesizeerrmsg + "<br/></span>");
                } else if (imageType != null && fileSize > 5) {
                    pageLayout.hideLoader();
                    componentContainer.find('.mrErrorText').remove();
                    componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\">" + imagefilesizeerrmsg + "<br/></span>");
                } else if (audioType != null && fileSize > 5) {
                    pageLayout.hideLoader();
                    componentContainer.find('.mrErrorText').remove();
                    componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\">" + audiofilesizeerrmsg + "<br/></span>");
                } else {
                    if (isThisIE && fileSize > 10) {
                        pageLayout.hideLoader();
                        componentContainer.find('.mrErrorText').remove();
                        componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\">" + ievideosizelimitaionmessage + "<br/></span>");
                    } else {
...
mediaupload.prototype.sendToWebServer = function(Base64String, fileType, fileExt, fileMediaType) {
    ...
    var request = $.ajax({
        type: 'POST',
        url: 'https://multimedia.tns-global.com/multimedia/L2/MediaUpload/POC/Service.asmx/UploadFileJSON',
        data: data,
