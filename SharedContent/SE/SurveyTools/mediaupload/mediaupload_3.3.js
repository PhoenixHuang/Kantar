...
mediaupload.prototype.getDependencies = function() {
    return [{
        ...
        'type': 'script',
        'url': surveyPage.path + 'lib/KO/MediaUpload/webcam/webcam.js'
    }];
}
...
mediaupload.prototype.render = function() {
	...
	var filesizelimit = this.getProperty("filesizelimit");
    ...
    var imagemediatypeerrmsg = this.getProperty("imagemediatypeerrmsg");
    var audiomediatypeerrmsg = this.getProperty("audiomediatypeerrmsg");
    var videomediatypeerrmsg = this.getProperty("videomediatypeerrmsg");
    var filesizeerrmsg = this.getProperty("filesizeerrmsg");
    var ievideosizelimitaionmessage = this.getProperty("ievideosizelimitaionmessage");
    var invalidMedia = this.getProperty("invalidMedia");
    var invalidMediaFile = this.getProperty("invalidMediaFile");
    ...
    var inputcontainer = $("<div class='col-sm-12 question-component'>");

    var mediatypeArray = mediatype.split(",");
    var valideMediaType = "";
    $.each(mediatypeArray, function(id, val) {
        var tempType = $.trim(val.toLowerCase());
        mediatypeArray[id] = tempType;
        if (tempType != 'image' && tempType != 'audio' && tempType != 'video') {
            valideMediaType = valideMediaType + " " + $.trim(val);
        }
    });

    //Removed duplicates
    mediatypeArray = unique(mediatypeArray);
    ...
    if (mediatypeArray.length == 1) {
        if (mediatypeArray[0] == 'image') {
            input.attr('accept', 'image/*;capture=camera');
        } else if (mediatypeArray[0] == 'video') {
            input.attr('accept', 'video/*;capture=camcorder');
        } else if (mediatypeArray[0] == 'audio') {
            input.attr('accept', 'audio/*;capture=microphone');
        }
    }

    if (mediatypeArray.length == 3) {
        if ($.inArray("image", mediatypeArray) !== -1 && $.inArray("audio", mediatypeArray) !== -1 && $.inArray("video", mediatypeArray) !== -1) {
            input.attr('accept', 'image/*;capture=camera,video/*;capture=camcorder,audio/*;capture=microphone');
        }
    }

    if (mediatypeArray.length == 2) {
        if ($.inArray("image", mediatypeArray) !== -1 && $.inArray("audio", mediatypeArray) !== -1) {
            input.attr('accept', 'image/*;capture=camera,audio/*;capture=microphone');
        }
        if ($.inArray("image", mediatypeArray) !== -1 && $.inArray("video", mediatypeArray) !== -1) {
            input.attr('accept', 'image/*;capture=camera,video/*;capture=camcorder');
        }
        if ($.inArray("audio", mediatypeArray) !== -1 && $.inArray("video", mediatypeArray) !== -1) {
            input.attr('accept', 'video/*;capture=camcorder,audio/*;capture=microphone');
        }
    }
    ...
    if (mediatype == "NotDefined" || opconame == "NotDefined" || passcode == "NotDefined") {
        ...
        if (valideMediaType == "") {
            ...
            this.nativeContainer.find('.mrErrorText').remove();
            this.nativeContainer.find('.mrQuestionText').before("<span class=\"mrErrorText\">" + invalidMedia + " " + valideMediaType + "<br/></span>");
        }
    ...
    if (PC_OTHERDEVICE == 'PC' || PC_OTHERDEVICE == 'OTHERDEVICE') {
        ...
        if (webcamaccess) {
            ...
            if (backendValue == "") {
                var webcamSWF = surveyPage.path + "lib/KO/MediaUpload/webcam/webcam.swf";
    ...
    $('#' + fileId).change({
        ...
        if (selectedfile != undefined) {
            ...
            var allowMedia = mediaTypeValidation(mediatypeArray, imageType, audioType, videoType);
            if (allowMedia == true) {
                if (fileSize > filesizelimit) {
                    pageLayout.hideLoader();
                    componentContainer.find('.mrErrorText').remove();
                    componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\">" + filesizeerrmsg + "<br/></span>");
                } else {
                ...
                pageLayout.hideLoader();
                componentContainer.find('.mrErrorText').remove();
                componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\">" + invalidMediaFile + " " + allowMedia + "</span><br/>");
            }
        } else {
            pageLayout.hideLoader();
        }
...
mediaupload.prototype.clickUpload = function() {
    ...
    $("#loader").append("<div id='progressbar' style='width:" + progressbarwidth + "%; margin:auto;font-size:" + loadingmessagefontsize + "px;'><div  class='progress-label' style='width:" + progressbarwidth + "%; position: absolute;text-align: center; margin: 7px 0 0 0;'>" + loadingmessage + "</div></div><br/><br/><br/>");
...
mediaupload.prototype.sendToWebServer = function(Base64String, fileType, fileExt, fileMediaType) {
    ...
    var loadingmessage = this.getProperty("loadingmessage");
    var webcamaccess = this.getProperty("webcam");
	var cdnpath = this.getProperty("cdnpath");
    ...
    var request = $.ajax({
        type: 'POST',
        url: cdnpath + '/multimedia/L2/MediaUpload/POC/Service.asmx/UploadFileJSON',
        data: data,
        ...
        success: function(data) {
            ...
            that.componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\" style=\"color:green;\">" + successmessage + "<br/></span>");
        },
        error: function(xhr, status, thrown) {
            ...
            if (status == 'timeout') {
                that.componentContainer.find('.mrErrorText').remove();
                that.componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\">" + status.toUpperCase() + ":" + timeoutmessage + "</span><br/>");
            } else {
                that.componentContainer.find('.mrErrorText').remove();
                that.componentContainer.find('.mrQuestionText').prepend("<span class=\"mrErrorText\">" + status.toUpperCase() + ":" + thrown + "," + errormessage + "<br/></span>");
            }
...
mediaupload.prototype.toolOptions = function() {
    $.extend(this.options, this.options.mediaupload);
    ...
    return {
        ...
        /*'imagemediatypeerrmsg': 'Please upload image file only.',
				'audiomediatypeerrmsg': 'Please upload audio file only.',
				'videomediatypeerrmsg': 'Please upload video file only.',*/
        'filesizeerrmsg': 'Media file size too large. Please ensure your file is less than 100MB.',
        ...
        'invalidMedia': 'Invalid media type(s):',
        'invalidMediaFile': 'Invalid media files, please provide',
        'webcam': false,
        ...
        'progressbarwidth': 50,
        'filesizelimit': 100,
		'cdnpath':'https://multimedia.kantaroperations.com/8016F8B/origin.tns-global.com'
    }
}
...
function mediaTypeValidation(mediatypeArray, imageType, audioType, videoType) {
    // For mobile, enable mobile native feature
    if (mediatypeArray.length == 1) {
        if (mediatypeArray[0] == 'image') {
            if (imageType == null) {
    ...
    } else if (mediatypeArray.length == 3) {
        if (imageType == null && audioType == null && videoType == null) {
            return 'image, video or audio';
        } else {
            return true;
        }
    } else if (mediatypeArray.length == 2) {
        if ($.inArray("image", mediatypeArray) !== -1 && $.inArray("audio", mediatypeArray) !== -1) {
            if (imageType == null && audioType == null) {
        ...
        if ($.inArray("audio", mediatypeArray) !== -1 && $.inArray("video", mediatypeArray) !== -1) {
            if (audioType == null && videoType == null) {
