...
clickableimages.prototype.getDependencies = function(){
    return [
        {'type':'script', 'url' : surveyPage.path + 'lib/qstudio/qcreator/qcomponent/RowPicker/RowPicker_2.4.1.js'}
...
clickableimages.prototype.build = function(){  
        ...
        $.each(this.inputs, function (i, e) {
			...
			if(typeof surveyPlatform=="undefined"){
			    ...
			    var tooltipText=label.find('abbr').attr("title");
			    //to empty abbr tag
			    label.find('abbr').attr("title","");
				var labelTxt = label.html();
			}else if(surveyPlatform=="Nfield"){
				var labelTxt = label.html().split("|")[0]  // this gets the styles from the html
			}
            // Create the row array
            rowArray.push(
                {	id:el.attr('value'),
                    label: labelTxt,
                    description:(typeof tooltipText!='undefined')?tooltipText:label.text(),  // just grab the text for tooltip purposes
        ...
        this.component.params(finalprops);
