...
dragndrop.prototype.build = function() {
    ...
    $.each(this.columnheaders, function(i, e) {
        ...
        if (that.getProperty("columnGrid$formattype") == "scale")
            colArray[i].subtitle = that.scale++;

        if (that.getProperty("column$displaytitle") == false && typeof colArray[i].image !== 'undefined')
            colArray[i].title = "";
    ...
    if ((this.getProperty("answertype") != null && (this.getProperty("answertype") == "MB" || this.getProperty("answertype") == "MB-pag")) || (this.getProperty("answertype") != null && this.columnheaders.length < 7)) {
        ...
        function loadrules() {
            ...
            var fnstring = "dd_" + that.getProperty("answertype").replace(/-/g, "_");
            ...
            var updateOptions = function(data) {
                ...
                data.s.bucketGrid.extrasmall.vgap = 1;
                data.s.bucketGrid.small.vgap = 1;
                data.s.bucketGrid.medium.vgap = 1;
                data.s.bucketGrid.large.vgap = 1;
                data.s.bucketGrid.extralarge.vgap = 1;
                data.s.bucketGrid.extrasmall.hgap = 1;
                data.s.bucketGrid.small.hgap = 1;
                data.s.bucketGrid.medium.hgap = 1;
                data.s.bucketGrid.large.hgap = 1;
                data.s.bucketGrid.extralarge.hgap = 1;
                ...
                $.extend(true, that.options, surveyPage.themeRuleSetprops.dragndrop);
...
dragndrop.prototype.toolOptions = function() {
    $.extend(this.options, this.options.dragndrop);
    return {
        row: {
            issingle: true,
            lockpos: false,
