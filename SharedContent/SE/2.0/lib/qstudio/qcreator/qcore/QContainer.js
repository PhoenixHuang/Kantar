...
QLayoutAbstract.prototype = {
    config: function(value) {
        ...
        this._params.border_color = QUtility.paramToHex(parseInt(value.border_color, 10)) ||
        ...
        this._params.bckgrnd_color = QUtility.paramToHex(parseInt(value.bckgrnd_color, 10)) ||
        ...
        this._params.label_fontcolor = QUtility.paramToHex(parseInt(value.label_fontcolor, 10)) ||
        ...
        if (target) {
            ...
            var addHelper = function(child) {
                ...
                if (!ownrow) {
                    ...
                    (isHorz) ?
                        cache.maxRowSize = Math.max(cache.maxRowSize, childWidth):
                        cache.maxSize = Math.max(cache.maxRowSize, childWidth);
                }

                // Resize Layout Container Height
                if (params.autoHeight) {
                    var defLayoutHeight = (isHorz) ? cache.yPosition + cache.maxSize : cache.maxColSize,
                        ownRowHeight = (!ownRowLayout.children.length) ?
                            0 : $(ownRowLayout).outerHeight(true);
                ...
                if (params.autoWidth) {
                    var defLayoutWidth = (isHorz) ? cache.maxRowSize : (cache.xPosition + cache.maxSize);
                    label.style.width = (defLayoutWidth + (params.border_width + params.padding - params.label_padding)*2) + "px";
                    layoutContain.style.width = defLayoutWidth + "px";
                }
...
QBaseLayout.prototype._layoutChildHorizontally = function(child_wrap) {
    ...
    curRowContain.style.width = cache.maxRowSize + "px";
    ...
    defaultLayout.style.width = curRowContain.style.width;
};
QBaseLayout.prototype._layoutChildVertically = function(child_wrap) {
    ...
    curRowContain.style.height = cache.maxColSize + "px";
    ...
    defaultLayout.style.height = curRowContain.style.height;
};
