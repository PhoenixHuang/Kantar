...
QBaseBtn.prototype.update = function() {
    ...
    if (!params.show_bckgrnd_import) {
        ...
        $(background).css( {
            ...
            'background-size': (width_padding + 'px ' + height_padding + 'px'),
    ...
    if (label.style.display !== "none") {
        ...
        label.style.width = ((!params.label_ovr_width) ?
            ((params.label_placement.indexOf("overlay") === -1) ? params.width : width_padding - params.label_overlay_padding*2) : params.label_width) + "px";
        label.style.padding =
        ...
        if (params.label_placement.indexOf("overlay") === -1) {
            ...
            switch (params.label_placement) {
                case "top overlay":
                    label.style.top = (-height_padding + params.label_top - border_width) + "px";
                    ...
                    break;
                case "center overlay":
                    label.style.top = ((-height_padding - labHeight)*0.5 + params.label_top - border_width) + "px";
                    break;
                default:
                    label.style.top = (-labHeight + params.label_top - border_width) + "px";
    ...
    if (params.show_stamp && jQuery.trim(params.stamp_import) !== "") {
        $(stamp).css( {
            'top': (((height_padding + border_width*2 - params.stamp_height) * 0.5) + params.stamp_top + labelOffset) + "px",
            'left': (((width_padding + border_width*2 - params.stamp_width) * 0.5) + params.stamp_left) + "px",
    ...
    if (jQuery.trim(params.image).length > 0) {
        ...
        $(image).on("load", function() {
            ...
            imageContain.style.top = (((height_padding + border_width*2 - $(this).height()) * 0.5) + labelOffset) + "px";
    ...
    wrap.style.display = "";
    wrap.style.width = (Math.max($(background).outerWidth(), $(label).outerWidth())) + "px";
    wrap.style.height = ($(background).outerHeight() + labHeight) + "px";
...
QFlowBtn.prototype.update = function() {
    ...
    label.style.fontSize = QUtility.convertPxtoEM(params.label_fontsize) + "em";
    label.style.color = "#" + params.label_fontcolor_up;
    label.style.width = params.width + "px";
    label.style.height = "auto";
    ...
    if (labHeight >= params.height) {
        label.style.height = params.height + "px";
        label.style.overflow = "hidden";
    } else {
        label.style.top = (((height_padding - labHeight)*0.5)) + "px";
    }
    ...
    if (jQuery.trim(params.image).length > 0) {
        ...
        $(image).on("load", function() {
            ...
            imageContain.style.visibility = "";
            imageContain.style.top = ((params.height - imageHeight)*0.5) + "px";
            label.style.width = (params.width - imageWidth - labOffset) + "px";
            ...
            if (labHeight >= params.height) {
                ...
                label.style.top = (((height_padding - labHeight)*0.5) + border_width) + "px";
            }
...
QTextBtn.prototype.update = function() {
    ...
    label.style.textAlign = (!params.isRTL) ? params.label_halign : "";
    label.style.fontSize = QUtility.convertPxtoEM(params.label_fontsize) + "em";
    label.style.width = ((!params.label_ovr_width) ? params.width : params.label_width) + "px";
    label.style.height = "auto";
    label.style.color = "#" + params.label_fontcolor_up;
    label.style.wordWrap = "break-word";
    ...
    if (params.txtbtn_trim) { height_padding = labHeight + params.padding * 2; }
    // Append back to widget
    wrap.insertBefore(label, stamp);
    label.style.left = (((width_padding + border_width*2 - labWidth) * 0.5) + params.label_left) + "px";
    label.style.top = (((height_padding + border_width*2 - labHeight) * 0.5) + params.label_top) + "px";
    label.style.visibility = "";
...
QKantarBaseBtn.prototype.update = function() {
    ...
    if (!params.show_bckgrnd_import) {
        ...
        $(background).css( {
            'background-repeat': 'no-repeat',
            'background-size': (labWidth + width_padding + 'px ' + height_padding + 'px'),
            'background-position': 'center',
            'background-image': (QUtility.ieVersion() < 9) ?
                "url(" + params.blank_gif + ") " + 'url('+params.bckgrnd_import_up+')' : 'url('+params.bckgrnd_import_up+')',
            'filter': "inherit progid:DXImageTransform.Microsoft.AlphaImageLoader(src="+params.bckgrnd_import_up+",sizingMethod='scale')"
...
QBaseInputBtn.prototype.update = function() {
    ...
    var doc = document,
        ...
        height_padding = params.height + params.padding * 2,
        border_width = params.border_width_up,
        txtarea_width = (params.width * 0.80),
        txtarea_height = (params.height * 0.80),
        minRegExp = /\bmin\b/ig,
        maxRegExp = /\bmax\b/ig,
        labHeight = 0,
        txtAreaEvent = (!QUtility.isMSTouch()) ?
    ...
    if ((params.width & 1) !== (txtarea_width & 1)) { txtarea_width += 1; }
    if ((params.height & 1) !== (txtarea_height & 1)) { txtarea_height += 1; }
    ...
    textarea.style.top = (((height_padding + border_width*2 - txtarea_height-2) * 0.5) +
        ((params.label_placement === 'top') ? labHeight : 0)) + "px";
    textarea.style.left = (((width_padding + border_width*2 - txtarea_width-2) * 0.5)) + "px";
    ...
    if (params.show_stamp && jQuery.trim(params.stamp_import) !== "") {
        $(stamp).css( {
            'top': (((height_padding + border_width*2 - params.stamp_height) * 0.5) + params.stamp_top + ((params.label_placement === 'top') ? labHeight : 0)) + "px",
            'left': (((width_padding + border_width*2 - params.stamp_width) * 0.5) + params.stamp_left) + "px",
            'width': params.stamp_width + "px",
            'height': params.stamp_height + "px",
            'background-repeat': 'no-repeat',
...
function QBaseFlagBtn(parent, configObj) {
    ...
    // Label CSS Style
    label.className = "qwidget_button_label";
    label.style.cssText = "position: absolute; filter: inherit; display: block; cursor: pointer;";
    if (QUtility.ieVersion() <= 8) { label.style.filter = ""; }    // Fix for IE8 Opacity/ClearType bug
    ...
    background.appendChild(stamp);
    wrap.appendChild(valueLabel);
    wrap.appendChild(background);
    wrap.appendChild(flagStick);
    wrap.appendChild(flagCircle);
    widget.appendChild(wrap);
    parentEle.appendChild(widget);
...
QBaseFlagBtn.prototype.update = function() {
    if (!this.widget()) { return false; }
    var that = this,
        doc = document,
        lightBoxInstance = QBaseLightBox.getInstance(),
        ...
        circleSize = 11,
        labHeight = 0,
        labcolor_hex_rgb = QUtility.hexToRgb(params.label_bckgrnd_color),
        labcolor_rgb_str = labcolor_hex_rgb.r+','+labcolor_hex_rgb.g+','+labcolor_hex_rgb.b,
        width_padding = params.width + params.padding * 2,
        height_padding = params.height + params.padding * 2;
    ...
    background.style.padding = params.padding + "px";
    background.style.backgroundColor = "#" + params.bckgrnd_color_up;
    background.style.border = params.border_width_up + "px " + params.border_style + " #" + params.border_color_up;
    ...
    // Flag Circle CSS Style
    flagCircle.style.borderRadius = flagCircle.style.webkitBorderRadius = flagCircle.style.mozBorderRadius = "50%";
    flagCircle.style.backgroundColor = "#" + params.flagcircle_color;
    flagCircle.style.top = (-circleSize*0.5) + "px";
    flagCircle.style.width = circleSize + "px";
    flagCircle.style.height = circleSize + "px";
    ...
    if (label.style.display !== "none") {
        ...
        label.style.textAlign = (!params.isRTL) ? params.label_halign : "";
        label.style.fontSize = QUtility.convertPxtoEM(params.label_fontsize) + "em";
        label.style.width = (width_padding - params.label_overlay_padding*2) + "px";
        label.style.height = "auto";
        label.style.padding = params.label_overlay_padding + "px";
        label.style.color = "#" + params.label_fontcolor_up;
        label.style.visibility = "hidden";
        label.innerHTML = params.label;
        // Temporarily append label to doc.body to calculate label width/height
        ...
        labHeight = $(label).outerHeight();
        // Append back to widget
        background.insertBefore(label, stamp);
        label.style.left = "0px";
        label.style.backgroundColor = (params.show_label_bckgrnd) ? ((QUtility.ieVersion() <= 8) ?
            '#' + params.label_bckgrnd_color : 'rgba('+labcolor_rgb_str+', 0.85)') : "";
        switch (params.label_placement) {
            case "top overlay":
                label.style.top = "0px";
                break;
            case "center overlay":
                ...
                break;
            default:
                label.style.top = (height_padding - labHeight) + "px";
                break;
        }
        label.style.visibility = "";
    }
    ...
    valueLabel.style.display = (params.show_value_label) ? "block" : "none";
    valueLabel.style.whiteSpace = "normal";
    valueLabel.style.wordWrap = "break-word";
    valueLabel.style.textAlign = (!params.isRTL) ? params.value_label_halign : "";
    valueLabel.style.fontSize = QUtility.convertPxtoEM(params.value_label_fontsize) + "em";
    valueLabel.style.width = width_padding + "px";
    valueLabel.style.height = "auto";
    valueLabel.style.color = "#" + params.value_label_fontcolor;
    valueLabel.style.border = params.border_width_up + "px " + params.border_style + " #" + params.border_color_down;
    valueLabel.style.backgroundColor = "#FFFFFF";
    valueLabel.style.visibility = "hidden";
    valueLabel.style.marginTop = params.border_width_up + "px";

    // Stamp CSS Style
    if (params.show_stamp && jQuery.trim(params.stamp_import) !== "") {
        $(stamp).css( {
            'top': (((height_padding - params.stamp_height) * 0.5) + params.stamp_top) + "px",
            'left': (((width_padding - params.stamp_width) * 0.5) + params.stamp_left) + "px",
            'width': params.stamp_width + "px",
            'height': params.stamp_height + "px",
            'background-repeat': 'no-repeat',
            'background-size': params.stamp_width + "px " + params.stamp_height + "px",
    ...
    if (params.use_lightbox) {
        ...
        if (zoomBtn && zoomBtn.nodeType === 1) {
            ...
            if (!zoomBtn.parentNode) {
                (actionType.indexOf("image") !== -1) ?
                    imageContain.appendChild(zoomBtn):
                    background.insertBefore(zoomBtn, stamp);
            }
        }
    } else {
        if (lightBoxInstance) { lightBoxInstance.remove(this); }
    ...
    if (jQuery.trim(params.image) !== "") {
        ...
        $(image).on("load", function() {
            ...
            this.style.top = params.img_top + "px";
            this.style.left = params.img_left + "px";
            imageContain.style.top = (((height_padding - $(this).height()) * 0.5)) + "px";
            imageContain.style.left = (((width_padding - $(this).width()) * 0.5)) + "px";
            imageContain.style.width = $(this).width() + "px";
            imageContain.style.height = $(this).height() + "px";
            ...
            $(this).off("load");
            background.insertBefore(imageContain, label);
            imageContain.style.visibility = "";
    ...
    wrap.style.display = "";
    wrap.style.width = $(background).outerWidth() + "px";
    wrap.style.height = ($(background).outerHeight() + $(flagStick).outerHeight()) + "px";
    widget.style.width = wrap.style.width;
    widget.style.height = wrap.style.height;
    flagStick.style.left = ((parseInt(widget.style.width, 10) - circleSize*0.5)*0.5) + "px";
    flagCircle.style.left = (parseInt(flagStick.style.left, 10)-((circleSize-5)*0.5)) + "px";
};
...
QBaseFlagBtn.prototype.valueLabel = function(value) {
    if (!this.widget()) { return null; }
    if (typeof value === "string" || typeof value === "number") {
        var valueLabel = this.cache().nodes.valueLabel;
...
QBaseFlagBtn.prototype.flagStickHeight = function(value) {
    if (!this.widget()) { return null; }
    var flagStick = this.cache().nodes.flagStick;
...
QBaseFlagBtn.prototype.animType = {
    ...
    image : {
        ...
        down : function(value) {
            var params = this.config(),
                ...
                valueLabel = cache.nodes.valueLabel,
                imageContain = cache.nodes.imageContain,
                image = imageContain.children[0],
                stamp = cache.nodes.stamp,
                img_width = (value) ? params.image_select_anim_width : params.width,
                img_height = (value) ? params.image_select_anim_height : params.height,
                circleSize = 11,
                width_padding = img_width + params.padding * 2,
                height_padding = img_height + params.padding * 2;

            // Background CSS Style
            background.style.width = img_width + "px";
            background.style.height = img_height + "px";
            ...
            // Stamp CSS Style
            if (stamp) {
                stamp.style.top = (((height_padding - params.stamp_height) * 0.5) + params.stamp_top) + "px";
                stamp.style.left = (((width_padding - params.stamp_width) * 0.5) + params.stamp_left) + "px";
            }

            // Image CSS Style
            imageContain.style.width = img_width + "px";
            imageContain.style.height = img_height + "px";
            if (image) {
                image.style.maxWidth = img_width + "px";
                image.style.maxHeight = img_height + "px";
                imageContain.style.top = (((height_padding - $(image).height()) * 0.5)) + "px";
                imageContain.style.left = (((width_padding - $(image).width()) * 0.5)) + "px";
            }
            ...
            // Flagstick and circle CSS Style
            flagStick.style.left = ((parseInt(widget.style.width, 10) - circleSize*0.5)*0.5) + "px";
            flagCircle.style.left = (parseInt(flagStick.style.left, 10)-((circleSize-5)*0.5)) + "px";
        }
    },
    pin : {
        hover : function(value) {
            var cache = this.cache(),
                wrapClone = cache.nodes.pin.children[3];

            $(wrapClone).stop(false, true);
            $(wrapClone)[(value) ? "fadeIn" : "hide"]();
            wrapClone.style.zIndex = (value) ? 2000 : "auto";
        },
        ...
        select : function(value) {
            var params = this.config(),
                ...
                pin = pinContain.children[1],
                arrw = pinContain.children[2],
                border_color = (value) ? params.border_color_down : params.border_color_up;
        ...
        down : function(value) {
            var params = this.config(),
                ...
                flagStick = cache.nodes.flagStick,
                pin = this._pinCreate(),
                wrap_width = (value) ? this.cache().pinSize.pinWidth : this.cache().pinSize.baseWidth,
                wrap_height = (value) ? this.cache().pinSize.pinHeight : this.cache().pinSize.baseHeight;
...
QBaseFlagBtn.prototype._pinCreate = function() {
    if (!this.cache().nodes.pin) {
        ...
        valueLabel.style.textAlign = params.value_label_halign;
        valueLabel.style.fontSize = QUtility.convertPxtoEM(params.value_label_fontsize) + "em";
        valueLabel.style.width = params.width + "px";
        valueLabel.style.marginLeft =(radius - params.width)*0.5 + "px";
        //valueLabel.style.width = radius + "px";
        valueLabel.style.height = "auto";
        valueLabel.style.color = "#" + params.value_label_fontcolor;
        // valueLabel.style.backgroundColor = "#FFF000";
        valueLabel.style.visibility = "hidden";
        ...
        // Append children
        contain.appendChild(valueLabel);
        contain.appendChild(pin);
        contain.appendChild(arrw);
        contain.appendChild(wrapClone);
        ...
        this.cache().nodes.pin = contain;
        this.cache().nodes.valueLabel = valueLabel;
        this.cache().pinSize = {
            pinWidth : radius,
        ...
        wrapClone.style.top = (-this.cache().pinSize.baseHeight + params.flagstick_height) + "px";
        wrapClone.style.left = ((radius - this.cache().pinSize.baseWidth)*0.5) + "px";
        $(wrapClone).on((!QUtility.isMSTouch()) ?
            "mousedown.dragnflag.pin touchstart.dragnflag.pin":
            (window.PointerEvent) ? "pointerdown.dragnflag.pin" : "MSPointerDown.dragnflag.pin", function(event) {
            event.preventDefault();
            event.stopPropagation();
        });
    }
...
QStudioSliderAbstract.prototype.slideInit = function(event) {
    ...
    handleContain[dragVar] = (event[pageVar] - handleContain[lastVar]*surveyScale);
...
QStudioSliderAbstract.prototype.slideMove = function(event) {
    ...
    var handleContain = this.cache().nodes.handleContain,
        ...
        pageVar = (isHorz) ? 'pageX' : 'pageY',
        styleVar = (isHorz) ? 'left' : 'top',
        moveLoc = (event[pageVar] - handleContain[dragVar])*(1/cache._surveyScale);
    ...
    $([track, highlight, tickContain]).on(clickEvent, function(event) {
        ...
        if (origEvent.type !== "touchmove") {
            if (origEvent.type !== 'touchstart' && origEvent.type !== "pointerdown" && origEvent.type !== "MSPointerDown") {
                ...
                handleContain[dragVar] = $(trackContain).offset()[(isHorz) ? "left" : "top"] + (track_border_width*surveyScale);
        ...
        if (wgtObj && wgtObj instanceof QStudioDCAbstract && wgtObj.type() !== "slider") {
            var that = this,
                ...
                zoomEvent = (!QUtility.isMSTouch()) ?
                    ((!QUtility.isTouchDevice()) ? "click.lightbox" : "touchstart.lightbox touchend.lightbox touchmove.lightbox"):
                    ((!QUtility.isTouchDevice()) ? "click.lightbox" : ((window.PointerEvent) ? "pointerdown.lightbox pointerup.lightbox" : "MSPointerDown.lightbox MSPointerUp.lightbox"));
