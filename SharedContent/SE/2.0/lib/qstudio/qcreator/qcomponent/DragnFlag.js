    ...
    DragnFlag.prototype = {
        ...
        initParams: function() {
            return {
                ...
                rowBtnShowValueLabel: { name:"Row Value Label: Display", value:true, description:"If set true, a label will appear above row items when dragged on scale.", type:"bool", wgtref: 'rowbtn', wgtname:"show_value_label", order:69 },
                ...
                /**********************************/
                rowBtnValLabDispType: { name:"Row Btn: Value Label Display Type", value:"column label", description:"Type of data to display in the value label.", type:"combo", options:["column value", "column label", "range value", "none"], order:105, display: false },
        ...
        create: function(parent) {
            ...
            var that = this,
                ...
                isVert = (rowContainType.indexOf("vertical") !== -1),
                isSeq = (rowContainType.indexOf("sequential") !== -1),
            ...
            for (var i=0; i<clen; i+=1) {
                tickLabArray.push(columnArray[i].label);
            }
            ...
            for (var i=0, rlen=rowArray.length; i<rlen; i+=1) {
                ...
                // Init Row Drag Vars
                //rowWgt.widget().setAttribute("homeX", parseInt(rowWgt.widget().style.left, 10).toString());
                rowWgt.widget().setAttribute("homeX", "0");
                rowWgt.widget().setAttribute("lastX", rowWgt.widget().getAttribute("homeX"));
            ...
            track.style.width = (colSldrConfigObj.width + 5) + "px";    // 5 is equal to the flagStick width
            ...
            var trackEndPadding = 5,
                trackHeightOffset = params.colSldrTrackHeight.value*0.5 + track_border_width,
            ...
            $(compContain).on(downEvent, '.qwidget_button', function(event) {
                ...
                var touchEvent = (event.type.indexOf("touch") !== -1 && event.originalEvent.targetTouches) ?
                    ...
                    rowWrap = rowNode.parentNode,
                    rowIndex = parseInt(rowNode.getAttribute('rowIndex'), 10),
                    rowWgt = respRef[rowIndex].row,
                    wrapOffsetLeft = rowNode._offsetLeft,
                    wrapOffsetTop = rowNode._offsetTop,
                    sldrOffTop = null,
                    baseValue = null,
                    flagStickRangeVal = null,
                    rangeValue = null,
                    columnValue = respRef[rowIndex].columnValue,
                    isMove = false,
                    rowNodeWidth = $(rowNode).outerWidth(),
                    rtlOffset = 0,
                    surveyScale = QUtility.getQStudioSurveyScale();

                if (!rowWgt.isAnswered()) {
                    rowNode._origWidth = $(rowNode).outerWidth();
                    rowNode._offsetLeft = $(rowWrap).offset().left;
                    rowNode._offsetTop = $(rowWrap).offset().top;
                    wrapOffsetLeft = rowNode._offsetLeft;
                    wrapOffsetTop = rowNode._offsetTop;
                }
                ...
                track.top = $(track).offset().top*(1/surveyScale.d) + track_border_width;
                track.bot = track.top + $(track).height() - track_border_width;

                // Set offsetLeft value on rowNode to use w/ anchoring to slider track
                sldrOffTop = (($(slider).offset().top - wrapOffsetTop))*(1/surveyScale.d);
                rowNode.offLeft = (wrapOffsetLeft - $(track).offset().left)*(1/surveyScale.a);
                ...
                if (rowWgt.enabled()) {
                    ...
                    if (that.qStudioVar.isCompRTL && !isSeq && !isVert) {
                        rtlOffset = (rowNodeWidth - sizeCache[selectAnimType].width);
                        rowNode.offLeft += (rowNode._origWidth - sizeCache[selectAnimType].width);
                    }
                    ...
                    if (selectAnimType !== "basic" && !rowWgt.isAnswered()) {
                        rowNode.dragX = wrapOffsetLeft + (sizeCache[selectAnimType].width*0.5)*(surveyScale.a);
                        rowNode.dragY = wrapOffsetTop + (sizeCache[selectAnimType].height*0.5)*(surveyScale.d);
                        rowNode.style.left = ((touchEvent.pageX - rowNode.dragX - rtlOffset)*(1/surveyScale.a)) + "px";
                    ...
                    $(doc).on(moveEvent, function(event) {
                        ...
                        // Only allow user to drag within component window area
                        var touchEvent = (event.type.indexOf("touch") !== -1 && event.originalEvent.targetTouches) ?
                            event.originalEvent.targetTouches[0] : ((!isMSTouch) ? event : event.originalEvent);
                        ...
                        if (touchEnabled && selectAnimType === "pin" && curPinSelect) {
                            var prevRowNode = curPinSelect.parentNode,
                                ...
                                prevRowWgt = respRef[prevRowIndex].row;
                        ...
                        var ex = (touchEvent.pageX - rowNode.dragX - rtlOffset),
                        ...
                        rowNode.setAttribute("lastX", (ex*(1/surveyScale.a)).toString());
                        rowNode.setAttribute("lastY", (ey*(1/surveyScale.d)).toString());

                        // Create Row Bounds and update on mousemove to determine if we are within the bounds of the slider track
                        rowNode.left = $(rowNode).offset().left*(1/surveyScale.a);
                        rowNode.right = rowNode.left;
                        rowNode.top = $(rowNode).offset().top*(1/surveyScale.d);
                        rowNode.bot = rowNode.top + sizeCache[selectAnimType].height + rowWgt.flagStickHeight();

                        if (!(rowNode.left > (track.right+trackEndPadding-sizeCache[selectAnimType].width*0.5) ||
                            rowNode.right < (track.left-trackEndPadding-sizeCache[selectAnimType].width*0.5) ||
                            rowNode.top > track.bot ||
                            rowNode.bot < track.top)) {
                            if (!rowWgt.isAnswered()) { rowWgt.isAnswered(true); }
                            baseValue = ((ex*(1/surveyScale.a)) - track_border_width + rowNode.offLeft + sizeCache[selectAnimType].width*0.5);
                            ...
                            rowNode.setAttribute(
                                "lastY",
                                ((sldrOffTop + trackHeightOffset - sizeCache[selectAnimType].height - rowWgt.flagStickHeight()-rowSldrTopOffset)).toString()
                            );
                    ...
                    $(doc).on(upEvent, function(event) {
                        ...
                        if (!rowWgt.isAnswered()) {
                            ...
                            rowNode.setAttribute("lastX", rowNode.getAttribute("homeX"));
                            rowNode.setAttribute("lastY", "0");
                            rowNode.style.left = rowNode.getAttribute("homeX") + "px";
                            rowNode.style.top = "";

                            // See if we need to unanswer row
                            if (respRef[rowIndex].columnValue !== -1) {
                                that.sendResponse(rowIndex, respRef[rowIndex].columnValue, false);
                                respRef[rowIndex].rangeValue = -1;
                                respRef[rowIndex].columnValue = -1;
                                if (isSeq) { rowContain.back(rowWgt); }
                            }
                        } else {
                            // Set row response
                            if (isSeq && respRef[rowIndex].columnValue === -1) { rowContain.next(); }
                            if (respRef[rowIndex].columnValue >= 0 && respRef[rowIndex].columnValue !== columnValue) {
                                // Un-answer current response
                                that.sendResponse(rowIndex, respRef[rowIndex].columnValue, false);
                            }
                            that.sendResponse(rowIndex, columnValue, true);
                            respRef[rowIndex].rangeValue = rangeValue;
                            respRef[rowIndex].columnValue = columnValue;
                        }

                        if (touchEnabled && selectAnimType === "pin" && !isMove) {
        ...
        getDimenResp: function() {
            ...
            for (var i=0, rlen=respRef.length; i<rlen; i+=1) {
                var row_id = respRef[i].row.widget().id,
                    rangeValue = respRef[i].rangeValue,
                    columnValue = respRef[i].columnValue;
