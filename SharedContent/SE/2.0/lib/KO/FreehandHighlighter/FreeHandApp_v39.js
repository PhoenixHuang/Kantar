...
var FreeHandApplication = function(){
	...
	function controllingBodyElements()
	{
        ...
        $(clearBtn).css({
            background: propertiesObj.buttonBGColor,
            color: propertiesObj.buttonTextColor,
        ...
        if (propertiesObj.clearButtonHide == "true") {
	...
	function setProperties()
	{
		...
		var isstudio,isonlnsrvy;
		isstudio = $(location).attr('href').search(/question\.htm/i) > 0; // Check for studio testing.
        isonlnsrvy=$(location).attr('href').search(/\.com/i) > 0;
		
		if(!propertiesObj.imgPath || propertiesObj.imgPath=="")
		alert("Image path is missing or not defined");
		else
		imgPath = propertiesObj.imgPath; 
	    ...
	    sharedContent=$('#sharedContent').attr('src');
		cursorPath = sharedContent+propertiesObj.cursorPath;
		cursorPath =cursorPath.replace(/\\/g,'\/');
		}
	...
	function updateSize()
	{
		...
		document.getElementById(propertiesObj.clearButton).value = propertiesObj.clearLabel;
        document.getElementById(propertiesObj.noOpnButton).value = propertiesObj.noOpinionLabel;
        document.getElementById(propertiesObj.showGridButton).value = propertiesObj.showLabel;
		//alert(columns);
	}
	//Clear the canvas
	...
	function visibleGrid()
	{
		...
		{
			document.getElementById(propertiesObj.showGridButton).value = propertiesObj.showLabe2
			showGrid = true
		}
		canvasObj.width = canvasObj.width
...
document.getElementsByName(propertiesObj.jsQid9)[0].value = data.slice(36000,40000)*/
        document.getElementById(propertiesObj.textAreaID).value = data
