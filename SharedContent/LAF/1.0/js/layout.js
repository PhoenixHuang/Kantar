...
var layout = function(){
	...
	// TODO: (mw) themePath has been hard coded to "LAF/Themes/"
    if(typeof tJSON!='undefined'){
	if (tJSON.themeSource == null ) { this.themePath = this.sharedContent + "LAF/Themes/" + tJSON.themeName + "/" + tJSON.themeVersion + "/"; } else { this.themePath = tJSON.themeSource + "/" + tJSON.themeName + "/" + tJSON.themeVersion + "/";}}
	else{
	this.themePath= this.sharedContent+"LAF/Themes/green/1.0/";
	}
...
layout.prototype = {
	...
	createLoader:function(){
		//var loadImage = "LAF/Layout/1.2/images/ajax-loader.gif";
		if (typeof tJSON!="undefined" ){
		var loadImage = "LAF/Themes/"+tJSON.themeName + "/" + tJSON.themeVersion +"/images/ajax-loader.gif";
		}
		else
		 var loadImage = "LAF/Themes/green/1.0/images/ajax-loader.gif";
		var loadImageURL = this.sharedContent;
		this.loader = $('<div>').attr("id","loader").css('display','none');//('none');
		this.content.after(this.loader);
		this.loader.append($('<img>').attr('src',loadImageURL+loadImage));
		this.showLoader();
	},
	showLoader:function(){
		this.content.hide();
    ...
    processInputAttributes:function(jInput,inputs,that){
        ...
        var qName = that.resolveHTMLValueToMDDName(jInput.attr("name"),type);
...
function loadjQueryUI(callback,context){
    ...
    css.attr({
        rel:  "stylesheet",
        type: "text/css",
        href: 'http://ajax.googleapis.com/ajax/libs/jqueryui/' + jQueryUIVersion + '/themes/' + jQueryUICSS + '/jquery-ui.min.css'
    });

    jQuery.ajax({
        type: "GET",
        url: 'http://ajax.googleapis.com/ajax/libs/jqueryui/' + jQueryUIVersion +'/jquery-ui.min.js',
