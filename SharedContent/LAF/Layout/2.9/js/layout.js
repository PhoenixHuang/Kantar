...
$(document).ready(function() {
    try {
        ...
        //----------------------------------------------------------------CSP
        //var testJetkins = $('.SEJson').html();
        if (typeof $('.SEJson').html() != 'undefined') {
			var tempSEJson = $('.SEJson').html().replace("<!-- ", "").replace("//-->", "");
			seJSON = JSON.parse((JSON.parse(JSON.stringify(tempSEJson))).replace(/\n/g, "").replace(/\t/g, " ").replace(/="/g, "='").replace(/"\//g, "'/").replace(/\\/g, "/").replace(/"\ /g, "'"));
            qJSON = seJSON.qJSON;
            engVersion = seJSON["engVersion"];
            manifestLoc = seJSON["manifestLoc"];
            preloadScript.push(seJSON["load"]);
        }

        if (typeof $('.projInfo').html() != 'undefined') {
            projectInfo = JSON.parse(JSON.parse(JSON.stringify($('.projInfo').html())));
            projectName = projectInfo["projectName"];
            projectVersion = projectInfo["projectVersion"];
            id = projectInfo["id"];
            serial = projectInfo["serial"];
            isTest = !!JSON.parse(String(projectInfo["isTest"]).toLowerCase());
            isDebug = projectInfo["isDebug"];
            cursorInOpens = projectInfo["cursorInOpens"];
            comp = projectInfo["comp"];
            deviceType = projectInfo["deviceType"];
            sampleSource = projectInfo["sampleSource"];
        }

        if (typeof $('.projInfo').html() != 'undefined') {
            tJSON = JSON.parse(JSON.parse(JSON.stringify($('.themeInfo').html())));
            imgCache = JSON.parse(JSON.parse(JSON.stringify($('.imgCache').html())));
            imageCacheBaseString = imgCache["imageCacheBaseString"];
        }
        //----------------------------------------------------------------------	
...
var layout = function() {
    ...
    this.navigations = $('#surveyButtons').css("display", "flex");
...
layout.prototype = {
    ...
    evaluateScripts: function(scripts, callback, context) {
        ...
        while (scripts.length > 0) {
            ...
            } else if (theScript == "initSurveyEngine();") {
                promises.push(initSurveyEngine());
            } else {
                if(typeof theScript != 'undefined'){
					console.log('Executing ' + theScript);
					promises.push(new Function(theScript)());
				}
            }
        }
    ...
    v3Styles: function() {
        ...
        this.allInputs.filter("input[type=radio],input[type=checkbox]").addClass("form-check-input");
        ...
        this.table.find("tbody tr td").each(function() {
            ...
            $(this).append("<label for='" + inputID + "' class='form-check-label'></label>");
        });



        var multicolumn = false;
        var dynamicgrid = false;
        if (typeof qJSON != 'undefined') {
            $.each(qJSON, function(index, json) {
        ...
        $("td[id^='Cell.']").on('click', function(e) {
            if (($(this).find("span[id^='Cell.']")).length < 1) {
        ...
        eachQuestion.each(function() {
            ...
            if (typeof ilabel != "undefined") {
                $(this).find("input[type=date], input[type=datetime-local], input[type=email], input[type=number], input[type=password], input[type=search-md], input[type=tel], input[type=text]:not(input[type='text'][otherinput='true']), input[type=time], input[type=url], textarea.md-textarea").each(function() {
                    $(this).after("<label for='" + $(this).attr('id') + "' class='form-check-label'>" + ilabel + "</label>");
                });
            }
    ...
    postLoadJS: function() {
        ...
        $(document).on("mousedown touchstart", "[data-toggle='popover']", function(e) {
            e.stopPropagation();
            $(this).webuiPopover({
                content: 'Content',
                type: 'html',
    ...
    defaultCategoricalRules: function() {
        ...
        if (this.questions.find("table").length < 1) {
            ...
            if (cells.length == 1 || cells.length == 2) {
                cells.each(function() {
                    ...
                    if ($(this).children('input').attr("otherid") != "") {
                        ...
                        if (isRTL) {
                            $(this).find('.md-form label').attr('style', 'text-align:right;');
                        } else {
    ...
    applyRTLStyle: function() {
		...
		$(".col-border").removeClass("waves-effect")
