...
var Webcam = {
	...
	dispatch: function() {
		...
		if (this.hooks[name] && this.hooks[name].length) {
			...
			document.querySelector('[id$="WebCamResp"]').innerHTML = "Error~"+args[0]+"~"+args[0];
			
			$('.takeSnap').css('background', '#ccc').attr("disabled", true);
			$('.fuploadButton').css('background', '#ccc').attr("disabled", true);
			
			$('.mrErrorText').remove();
			$('.mrQuestionText').append("<span class=\"mrErrorText\"><br/>"+args[0]+"</span>");
			return args[0];
		}
